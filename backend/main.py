...
def suggest(
    ...
    """
    股票代號 / 中英文名稱模糊搜尋（從 stocks_meta 抓）。
    - 有 q：symbol / name_zh / name_en / short_name_zh / short_name_en / twse_symbol 做 LIKE %q%
    - 沒 q：回傳前幾檔當預設熱門
    回傳欄位配合前端 renderSuggestions 使用。
    """
    try:
        cn = pool.get_connection()
        cur = cn.cursor(dictionary=True)
        ...
        if q:
            ...
            sql = """
                ...
                WHERE symbol         LIKE %s
                   ...
                   OR twse_symbol    LIKE %s
                ORDER BY symbol
                LIMIT %s
            """
            cur.execute(sql, (like, like, like, like, like, like, limit))
        else:
            # 沒打字 -> 給前幾檔當預設
            sql = """
                SELECT symbol,
        ...
        print("MySQL error in /api/suggest:", e)
        # 出錯就回空陣列，前端會顯示「無符合股票」或「查詢失敗」
        return []
    finally:
        try:
            cur.close(); cn.close()
